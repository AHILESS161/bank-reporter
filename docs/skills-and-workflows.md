...
Агентские возможности оформлены как типизированные skills, а бизнес-сценарии — как проверяемые workflows. Это внутренняя архитектура сервиса: пользователь работает с ней через естественные команды в чате. Она позволяет добавлять источник, расчёт или экспорт отдельно, явно задавать права и соединять совместимые блоки без переписывания оркестратора.
